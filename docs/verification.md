...
在 `/Applications/Daka.app` 上执行。工作日 09:00 后（或应下班时间后）启动会立即弹全屏催打卡，属预期行为。
...
- [ ] `launchctl print gui/$(id -u)/com.xue.daka.schedule` 能看到服务与三个 `StartCalendarInterval`（09:00 / 09:30 / 18:00）。
...
- [ ] 设置页「设置」分组只剩「考勤规则 / 工作日 / 系统与启动」三页；「考勤规则」含上班时间、弹性时间、工作时长三项 + 重复提醒。
- [ ] 上班窗口：09:00 触发全屏提醒；08:59 不触发。
- [ ] 弹性时间改为 15 分钟后，09:14 前上班卡正常，09:15 起推迟。
- [ ] 早到（如 08:40 打卡）→ 下班时间 18:00，18:00 开始提醒下班。
- [ ] 准点（09:00 打卡）→ 下班时间 18:00，18:00 开始提醒下班。
- [ ] 弹性内（09:16 打卡）→ 下班时间 18:16，18:16 开始提醒下班。
- [ ] 晚到（10:00 打卡）→ 下班时间 19:00，19:00 开始提醒下班（无上限）。
- [ ] 提前下班（18:10 打卡 < 18:16）→ 下班未完成，继续提醒。
- [ ] 满足工时后下班 → 全屏消失。
- [ ] 未打上班卡 → 只催上班，不催下班。
- [ ] 「工作时长」stepper 改为 8 小时 → 下班时间相应提前。
- [ ] 旧版 `data.json`（含 `morningWindowStart`/`morningDeadline`/`minWorkDurationHours`）能平滑迁移为新的考勤三参数，不触发损坏备份。
- [ ] `data.json` 重新写盘后只含 `workStartTime`/`workDurationHours`/`flexMinutes` 等新键，不含旧键。
- [ ] 健康提醒窗口为 09:00–18:00（工作时长内）：18:01 不触发。
