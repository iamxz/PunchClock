...
弹窗宽度 `220 → 160`，删除「上班打卡 / 下班打卡」两行，只保留标题栏与圆环按钮：
...
整行删除「上班打卡 / 下班打卡」两行（连同左侧勾/圈、右侧状态词与 `（N 次）`）。弹窗不再显示任何打卡状态文字；打卡状态由圆环（进度、颜色、圆心时间与时长）与系统提醒体现。

> 修订（2026-09-16，第二次）：初版保留两行只去时间，用户确认要**整行删除**。因此 `statusRow` 与 `scheduleInactive` 一并移除。
...
- `Sources/Daka/MenuBarView.swift`（改）：
  - 删除两行 `statusRow` 调用与 `statusRow` / `scheduleInactive` 定义。
  - 弹窗宽度 `220 → 160`。
