...
- **工作日判断**：基于标准日历，默认周一至周五，并内置 2025–2026 年中国法定节假日与调休数据（节假日自动休息、调休日自动补班）；「设置 → 工作日」以月历形式展示，可点击任意一天手动微调（个人请假 / 公司补班），微调以蓝色边框标记。
...
```jsonc
{
  "settings": {
    ...
    "workdayOverrides": {},               // "yyyy-MM-dd" -> 是否上班，手动微调覆盖默认判定
    "workStartTime": "09:00",
    "workDurationHours": 9,
    "flexMinutes": 30,
