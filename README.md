...
- **定点拉起**：通过 `launchd` 在四个打卡时点自动拉起已在后台退出的应用（单实例）；若应用已在运行则只激活，不弹主窗口。
...
- **关闭主窗口不退出应用**，应用继续在后台提醒；再次点 Dock 图标或菜单栏「打开 Daka」可重新打开。
...
- 定点拉起依赖 `launchd`，需要用户已登录且系统已唤醒；到点拉起在后台运行，不会弹出主窗口（应用已在运行时只激活，也不会弹窗）。
