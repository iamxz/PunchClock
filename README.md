...
- **时间段请假**：在日历上右键「新增请假」，可填当天几小时、也可跨天（周末/节假日一并覆盖，但只有工作日计入统计）。请假时长直接抵扣当天应上班时长：9:00 上班、工时 9 小时、请假 16:00–18:00，则 16:00 打下班卡即算正常出勤。日历上标「假」，tooltip 列出当天具体时段；请假期间不弹打卡与健康提醒，请假结束立刻恢复。半天按 0.5 天折算，统计页展示「本月请假 N 天」。
...
- **打卡统计**：主窗口「统计」页展示五张指标卡（本月打卡 / 连续打卡 / 平均上班 / 缺卡 / 本月请假）与最近 7 / 14 / 30 天每日上班时长柱状图。
...
```jsonc
{
  ...
  "leaves": [
    {
      "id": "8E3B0C6E-6F1A-4C2B-9E3D-8C1F2A3B4C5D",
      "start": "2026-09-14T16:00:00+08:00",
      "end": "2026-09-14T18:00:00+08:00",
      "label": "看牙"
    }
  ]
}
...
`records` 以 `yyyy-MM-dd` 为键保留全部历史；`leaves` 是跨天的请假区间（`start`/`end` 为绝对时刻，`label` 可选），每天有效时段由它与当天边界求交派生，取消请假会整条删除。文件损坏时会重命名为 `data.json.corrupt-<时间戳>-*` 备份，并重建空存储。

旧版用 `records[key].skipped: true` 表示「今天不打卡」。首次读取时它会迁移成一条整天请假，之后不再写出该键；**降级回旧版本将无法看到请假**（旧版只认 `skipped`）。
...
测试覆盖 `ScheduleEvaluator`（时间窗口/全屏强提醒/工作日/请假/开机强制）、`PunchStore`（往返、损坏恢复、跨天、请假增删改）、`Statistics`（含请假折算）等核心逻辑，均使用注入时钟，不依赖真实时间。
...
- 节假日数据为内置的 2025–2026 年中国法定节假日与调休表，不会自动更新；表外的日子按星期判断，需要休息请用「新增请假」。
